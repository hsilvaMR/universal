...
@section('content')
  <?php $arrayCrumbs = [ $cert->nome => route('certificationsPageB'),$process->referencia => route('processesPageB',$process->id_certificacao),trans('backoffice.allActivities') => route('activitiesPageB',$id_proc) ]; ?>
  @include('backoffice/includes/crumbs')

  <div class="page-titulo">{{ trans('backoffice.allActivities') }}</div>

  <a href="{{ route('activitiesNewPageB',$id_proc) }}" class="abt bt-verde modulo-botoes"><i class="fas fa-plus"></i> {{ trans('backoffice.addNew') }}</a>
          ...
          @foreach($dados as $val)
            <tr id="linha_{{ $val['id'] }}">
              ...
              <td class="table-opcao">
                <a href="{{ route('activitiesEditPageB',[$id_proc,$val['id']]) }}" class="table-opcao">
                  <i class="fas fa-pencil-alt"></i>&nbsp;{{trans('backoffice.Edit')}}
                </a>&ensp;
                <a href="{{ route('tasksPageB',$val['id']) }}" class="table-opcao">
                  <i class="fas fa-tasks"></i>&nbsp;{{trans('backoffice.Tasks')}}
                </a>&ensp;
...
<script>
  function updateOnOffTM(id){
    $.ajax({
      ...
      data: { tabela:'gest_atividades', id:id },
