...
@section('content')
  <?php $arrayCrumbs = [ trans('backoffice.allFAQs') => route('cheeseFAQsAllPageB') ]; ?>
  @include('backoffice/includes/crumbs')

  <div class="page-titulo">{{ trans('backoffice.allFAQs') }}</div>

  <a href="{{ route('cheeseFAQsNewPageB') }}" class="abt bt-verde modulo-botoes"><i class="fas fa-plus"></i> {{ trans('backoffice.addNew') }}</a>
  	...
  	<div class="modulo-scroll">
  	  <table class="modulo-body" id="sortable">
          ...
          <th class="display-none"></th>
  			  <th>#</th>
  			  <th>{{ trans('backoffice.Question') }}</th>
          ...
          <td class="check-small">
            ...
            <label for="check3{{ $art['id'] }}"><span></span></label>
  			  ...
  			  <td class="table-opcao">
  				<a href="{{ route('cheeseFAQsEditPageB',['id'=>$art['id']]) }}" class="table-opcao"><i class="fas fa-pencil-alt"></i>&nbsp;{{trans('backoffice.Edit')}}</a>&ensp;
...
<script type="text/javascript">
  ...
  function apagarLinha(){
    ...
    $.ajax({
      type: "POST",
      url: '{{ route('cheeseFAQsAllApagarB') }}',
      data: { id:id },
      headers:{ 'X-CSRF-Token':'{!! csrf_token() !!}' }
    })
    .done(function(resposta) {
