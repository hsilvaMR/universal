...
<table cellpadding="6" cellspacing="0">
 <tr>
  ...
  <td width="230" style="text-align:right; line-height:15px;">
  	<b><span style="font-size:11px;color:#002d73;">LA | PRODUTOS LÁCTEOS, LDA.</span><br></b>
  	<span style="font-size:9px;color:#1974D8;"> OLIVEIRA DE AZEMÉIS, PORTUGAL</span>
  </td>
  <td width="115" style="line-height:20px; margin-left:50px;"><span style="font-size:10px;color:#1974D8;">Rua do Pego 25,<br>3721-902 Travanca <br>Oliveira de Azeméis</span></td>
...
foreach($morada_armazem as $value){
	...
	foreach($enc_linha as $enc){
		if($enc['id_morada'] == $value->id){
			...
			$count_prod = $count_prod + count($enc['produto']);
			...
			if($count_prod % 2 == 0){
		  	...
		  	$valor_caixa = $enc['qtd_caixa']* $enc['valor'];
		    ...
		    $valor_com_iva = round($valor_total_t + $valor_iva, 2);
		  	...
		  	$html_tr.= '
			  	...
			  	<tr style="'.$color.'height:50px;">
			  		<td style="font-size:11px;height:50px;">'.$enc['produto'].'</td>
		            ...
		            <td style="font-size:11px;text-align:right;height:50px;">
		            	'.$valor_caixa.' € <br> 
		            	<span style="font-size:10px;">'.$enc['valor'].' €/'.trans('seller.article').'</span>
		            </td>
	...
	if ($valor_dec >0) {
		$html_morada.= '
			...
			<table cellpadding="6" cellspacing="0">
				...
				<tr style="line-height:20px;">
				  	<td style="border-top:1px solid #eee;border-bottom:1px solid #eee;font-size:11px;">
				  		<b>'.$value->nome_personalizado.' </b> '.$value->morada.', '.$value->codigo_postal.' '.$value->cidade.', '.$value->pais.'
				  	</td>
				</tr>
			</table>
...
if ($encomenda->telefone == 0) {
	$telefone = '';
}
else{
	$telefone = $encomenda->telefone.'<br> &ensp;';
}
...
$html = '
	<table cellpadding="6" cellspacing="0">
	 	<tr>
	 		<td><span style="font-size:14px;color:#1974D8;width:100px;">'.trans('seller.Order').' '.$encomenda->id_enc.' </span></td>
			...
			<td style="float:left;width:180px;">
				<table style="float:left;width:180px;">
					<tr>
						...
						<td style="border-left:1px solid #000;">  '.date('Y-m-d',$encomenda->data_enc).'</td>
					</tr>
				</table>
			</td>
			<td style="float:left;width:420px;">
				<table style="float:right;width:420px;">
					<tr style="float:left;width:420px;">
						...
						<td style="border-left:1px solid #000;">  '.$encomenda->nome.'<br> &ensp;'.$telefone .$encomenda->email.'</td>
					</tr>
				</table>
			</td>
		</tr>
...
//\DB::table('encomenda')->where('id',$encomenda->id_enc)->update(['documento','a']);
