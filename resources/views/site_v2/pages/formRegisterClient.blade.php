...
<div style="min-height:calc(100% - 192px);background:pink;position:relative;">
	...
	<div class="div-blue div-blue-cliente"></div>
		...
		<div class="row div-row">
			<div class="col-md-4" style="background-color:#fbfbfb;">
				<div class="div-left">
					...
					<li>{{ trans('site_v2.Seller_pg') }} *  <a href="{{ route('formRegisterSellerPageV2') }}"><span class="tx-navy">{{ trans('site_v2.Sign_up_here') }}.</span></a></li>
					<span style="font-size:12px;padding-left:25px;">* Apenas para venda de lacticínios </span>
				</div>	
			</div>
			...
			<div class="offset-md-1 col-md-7" style="background-color: #1974d8;">
				<div class="div-right">
					...
					<form id="form-register-client" action="{{ route('registerClientPost') }}" name="form" method="post">

            			{{ csrf_field() }}

						<label>{{ trans('site_v2.Name') }}</label>
						<input class="ip" type="text" name="nome" placeholder="{{ trans('site_v2.Required') }}">
						...
						<br>
						<button class="bt-gray" type="submit">{{ trans('site_v2.REGISTER') }}</button>
...
@section('javascript')
	...
	<script type="text/javascript">
		...
		$("#termos_cond").on('change', function() {
	  	...
	  	$('#form-register-client').on('submit',function(e) {
			$("#labelSucesso").hide();
			$("#labelErros").hide();
			$('#loading').show();
			$('#botoes').hide();
	        ...
	        .done(function(resposta){
		        ...
		        if(resp.estado=='sucesso'){
		          $('#id').val(resp.id);
		          $("#labelSucesso").show();
		          document.getElementById("form-register-client").reset();
		        }else if(resposta){
