      ...
      <div class="col-lg-9">
        <div class="bg-white">
          <div class="data-border-form">
            <label>{{ trans('site_v2.Billing_Address') }}</label>
          </div>
          <form id="updateAdressB" method="POST" enctype="multipart/form-data" action="{{ route('updateAdressBPostV2') }}">
            {{ csrf_field() }}
            <div class="data-form">
              
              <label>{{ trans('site_v2.Name') }}</label>
              <input class="ip data-ip" type="text" name="name" @if(isset($morada_faturacao->nome)) value="{{ $morada_faturacao->nome }}" @else value="" @endif>
          
              <div class="row">
                <div class="col-md-6">
                  <label>{{ trans('site_v2.E-mail') }}</label>
                  <input class="ip data-ip" type="email" name="email" @if(isset($morada_faturacao->email)) value="{{ $morada_faturacao->email }}" @else value="" @endif>
                </div>
                <div class="col-md-3">
                  <label>{{ trans('site_v2.Phone_call') }}</label>
                  <input class="ip data-ip" type="text" name="contacto" placeholder="{{ trans('site_v2.optional') }}" @if(isset($morada_faturacao->telefone)) value="{{ $morada_faturacao->telefone }}" @else value="" @endif>
                </div>
                <div class="col-md-3">
                  <label>{{ trans('site_v2.VAT') }}</label>
                  <input class="ip data-ip" type="text" name="nif" maxlength="9" placeholder="{{ trans('site_v2.optional') }}" @if(isset($morada_faturacao->nif) && $morada_faturacao->nif != 0) value="{{ $morada_faturacao->nif }}" @else value="" @endif>
                </div>
              </div>

              <label>{{ trans('site_v2.Address') }}</label>
              <input class="ip data-ip" type="text" name="morada" @if(isset($morada_faturacao->morada)) value="{{ $morada_faturacao->morada }}" @else value="" @endif>
              <input class="ip data-ip" type="text" name="morada_opc" placeholder="{{ trans('site_v2.optional') }}" @if(isset($morada_faturacao->morada_opc)) value="{{ $morada_faturacao->morada_opc }}" @else value="" @endif>

              <div class="row">
                <div class="col-md-3">
                  <label>{{ trans('site_v2.Postal_Code') }}</label>
                  <input id="code-postal" maxlength="11" class="ip data-ip" type="text" name="codigoPostal" @if(isset($morada_faturacao->codigo_postal)) value="{{ $morada_faturacao->codigo_postal }}" @else value="" @endif>
                </div>
                <div class="col-md-4">
                  <label>{{ trans('site_v2.City') }}</label>
                  <input class="ip data-ip" type="text" name="cidade" @if(isset($morada_faturacao->cidade)) value="{{ $morada_faturacao->cidade }}" @else value="" @endif>
                </div>
                <div class="col-md-5">
...
@section('javascript')
  ...
  <script>
    $('#updateAdressB').on('submit',function(e) {
