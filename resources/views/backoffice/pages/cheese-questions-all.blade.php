...
@section('content')
  <?php $arrayCrumbs = [ trans('backoffice.allQuestions') => route('cheeseQuestionsAllPageB') ]; ?>
  @include('backoffice/includes/crumbs')

  <div class="page-titulo">{{ trans('backoffice.allQuestions') }}</div>

  <a href="{{ route('cheeseQuestionsNewPageB') }}" class="abt bt-verde modulo-botoes"><i class="fas fa-plus"></i> {{ trans('backoffice.addNew') }}</a>
          ...
          <td class="check-small">
            ...
            <label for="check3{{ $art['id'] }}"><span></span></label>
  			  ...
  			  <td class="table-opcao">
  				<a href="{{ route('cheeseQuestionsEditPageB',['id'=>$art['id']]) }}" class="table-opcao"><i class="fas fa-pencil-alt"></i>&nbsp;{{trans('backoffice.Edit')}}</a>&ensp;
...
<script type="text/javascript">
  ...
  function apagarLinha(){
    ...
    $.ajax({
      type: "POST",
      url: '{{ route('cheeseQuestionsAllApagarB') }}',
      data: { id:id },
      headers:{ 'X-CSRF-Token':'{!! csrf_token() !!}' }
    })
    .done(function(resposta) {
