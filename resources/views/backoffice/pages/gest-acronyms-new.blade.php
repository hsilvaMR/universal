...
@section('content')
  @if($funcao=='new')<?php $arrayCrumbs = [ trans('backoffice.allAcronyms') => route('acronymsPageB'), trans('backoffice.newAcronym') => route('acronymsNewPageB') ]; ?>@else
  <?php $arrayCrumbs = [ trans('backoffice.allAcronyms') => route('acronymsPageB'), trans('backoffice.editAcronym') => route('acronymsEditPageB',['id'=>$dados->id]) ]; ?>@endif
  @include('backoffice/includes/crumbs')

  <div class="page-titulo">@if($funcao=='new'){{ trans('backoffice.newAcronym') }}@else{{ trans('backoffice.editAcronym') }}@endif</div>

  <form id="form" method="POST" enctype="multipart/form-data" action="{{ route('acronymsFormB') }}">
    {{ csrf_field() }}
    <input type="hidden" id="id" name="id" value="@if(isset($dados->id)){{ $dados->id }}@endif">
    <div class="row">
      <div class="col-lg-4">
        <label class="lb">{{ trans('backoffice.Initials') }}</label>
        <input class="ip" type="text" name="sigla" value="@if(isset($dados->sigla)){{ $dados->sigla }}@endif">
      </div>
      <div class="col-lg-8">
        <label class="lb">{{ trans('backoffice.Description') }}</label>
        <input class="ip" type="text" name="descricao" value="@if(isset($dados->descricao)){{ $dados->descricao }}@endif">
      </div>
    </div>
    ...
    <div id="botoes">
      ...
      <a href="{{ route('acronymsPageB') }}" class="abt bt-vermelho float-right"><i class="fas fa-times"></i> {{ trans('backoffice.Cancel') }}</a>
    </div>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <a href="{{ route('acronymsPageB') }}" class="abt bt-cinza"><i class="fas fa-arrow-left"></i> {{ trans('backoffice.Back') }}</a>&nbsp;
