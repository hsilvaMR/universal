...
@section('content')
  <?php $arrayCrumbs = [ trans('backoffice.allArticles') => route('blogAllPageB') ]; ?>
  @include('backoffice/includes/crumbs')

  <div class="page-titulo">{{ trans('backoffice.allArticles') }}</div>

  <a href="{{ route('blogNewPageB') }}" class="abt bt-verde modulo-botoes"><i class="fas fa-plus"></i> {{ trans('backoffice.addNew') }}</a>
          ...
          <td>{{ $art['id'] }}</td>
          	...
          	<label for="check3{{ $art['id'] }}"><span></span></label>
  			  ...
  			  <td class="table-opcao">
  				<a href="{{ route('articlePageV2',['lang'=>$lingua, 'id'=>$art['id'], 'slug'=>$art['slug']]) }}" class="table-opcao" target="_blank"><i class="fas fa-search"></i>&nbsp;{{trans('backoffice.View')}}</a>&ensp;
          <a href="{{ route('blogEditPageB',['id'=>$art['id']]) }}" class="table-opcao"><i class="fas fa-pencil-alt"></i>&nbsp;{{trans('backoffice.Edit')}}</a>&ensp;
  				<span class="table-opcao" onclick="$('#id_modal').val({{ $art['id_pai'] }});" data-toggle="modal" data-target="#myModalDelete">
  				  <i class="fas fa-trash-alt"></i>&nbsp;{{trans('backoffice.Delete')}}
  				</span>
  			  </td>
  			</tr>
  		  @endforeach
  		  @if(empty($artigos)) <tr><td colspan="7">{{ trans('backoffice.noRecords') }}</td></tr> @endif
  	  	</tbody>
  	  </table>
  	</div>
  </div>
...
<script type="text/javascript">
  function updateOnOffTM(id){
    $.ajax({
      type: "POST",
      url: '{{ route('updateOnOffTM') }}',
      data: { tabela:'blog', referencia:'id_pai', id:id },
