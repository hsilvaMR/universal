...
@section('content')
  <?php $arrayCrumbs = [ trans('backoffice.allCertifications') => route('certificationsPageB') ]; ?>
  @include('backoffice/includes/crumbs')

  <div class="page-titulo">{{ trans('backoffice.allCertifications') }}</div>

  @if(isset($tipo_cert_criar))
    <a href="{{ route('certificationsNewPageB') }}" class="abt bt-verde modulo-botoes"><i class="fas fa-plus"></i> {{ trans('backoffice.addNew') }}</a>
  @endif
          ...
          @foreach($dados as $val)
            <tr id="linha_{{ $val['id'] }}">
              ...
              <td class="table-opcao">
                <a href="{{ route('certificationsEditPageB',$val['id']) }}" class="table-opcao">
                  <i class="fas fa-pencil-alt"></i>&nbsp;{{trans('backoffice.Edit')}}
                </a>&ensp;
                <a href="{{ route('processesPageB',$val['id']) }}" class="table-opcao">
                  <i class="fas fa-tasks"></i>&nbsp;{{trans('backoffice.Processes')}}
                </a>&ensp;
...
<script>
  function updateOnOffTM(id){
    $.ajax({
      ...
      data: { tabela:'gest_certificacoes', id:id },
