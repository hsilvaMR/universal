...
@section('content')
  <?php $arrayCrumbs = [ $cert->nome => route('certificationsPageB'), $process->referencia => route('processesPageB',$process->id_certificacao), $acti->referencia => route('activitiesPageB',$acti->id_processo),trans('backoffice.allTasks') => route('tasksPageB',$id_acti) ]; ?>
  @include('backoffice/includes/crumbs')

  <div class="page-titulo">{{ trans('backoffice.allTasks') }}</div>

  <a href="{{ route('tasksNewPageB',$id_acti) }}" class="abt bt-verde modulo-botoes"><i class="fas fa-plus"></i> {{ trans('backoffice.addNew') }}</a>
          ...
          @foreach($dados as $val)
            <tr id="linha_{{ $val['id'] }}">
              ...
              <td class="table-opcao">
                <a href="{{ route('tasksEditPageB',[$id_acti,$val['id']]) }}" class="table-opcao">
...
<script>
  function updateOnOffTM(id){
    $.ajax({
      ...
      data: { tabela:'gest_tarefas', id:id },
