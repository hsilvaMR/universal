...
@section('content')
  <?php $arrayCrumbs = [ trans('backoffice.allSlides') => route('webSlideAllPageB') ]; ?>
  @include('backoffice/includes/crumbs')

  <div class="page-titulo">{{ trans('backoffice.allSlides') }}</div>

  <a href="{{ route('webSlideNewPageB') }}" class="abt bt-verde modulo-botoes"><i class="fas fa-plus"></i> {{ trans('backoffice.addNew') }}</a>
          ...
          <td>{{ $art['id'] }}</td>
          	...
          	<label for="check3{{ $art['id'] }}"><span></span></label>
  			  ...
  			  <td class="table-opcao">
  				<a href="{{ route('webSlideEditPageB',['id'=>$art['id']]) }}" class="table-opcao"><i class="fas fa-pencil-alt"></i>&nbsp;{{trans('backoffice.Edit')}}</a>&ensp;
...
<script type="text/javascript">
  function updateOnOffTM(id){
    $.ajax({
      ...
      data: { tabela:'conteudos_slide', id:id },
